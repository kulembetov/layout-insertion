...
# Place BlockTypeUtils before EnhancedFigmaExtractor so it is always in scope
class BlockTypeUtils:
    ...
    def detect_block_type(node: dict) -> Tuple[str, str]:
        ...
        sorted_patterns = sorted(config.FIGMA_TO_SQL_BLOCK_MAPPING.items(), key=lambda x: len(x[0]), reverse=True)
...
class BlockUtils:
    ...
    def build_block_dict(block, slide_config: dict = None) -> dict:
        ...
        if get('sql_type') in text_block_types and slide_config and get('sql_type') in slide_config:
            all_colors = []
            all_fonts = []
...
# Place ColorUtils before EnhancedFigmaExtractor so it is always in scope
class ColorUtils:
    @staticmethod
                    ...
                    for color_group in block.get('children', []):
                        ...
                        for text_child in color_group.get('children', []):
                            if text_child.get('type') == 'TEXT':
                                ...
                                obj['fontFamily'] = FontUtils.normalize_font_family(font_family)
...
class FigmaToSQLIntegrator:
    ...
    def generate_sql_for_slides(self, slide_numbers: List[int], output_dir: str = "sql_output"):
        ...
        LogUtils.log_block_event("\nProcessing complete!")
        LogUtils.log_block_event(f"   Extracted {len(figma_data.get('slides', []))} slides")
...
if __name__ == "__main__":
    ...
    if args.mode == 'slides' and args.slides:
        ...
        LogUtils.log_block_event(f"   Total blocks analyzed: {validation.get('total_blocks', 0)}")
        LogUtils.log_block_event(f"   Slides analyzed: {validation.get('slides_analyzed', 0)}")
        LogUtils.log_block_event(f"   Font weight distribution: {validation.get('weight_distribution', {})}")
        
        invalid = validation.get('invalid_weights_found', [])
        if invalid:
            LogUtils.log_block_event(f"   Found {len(invalid)} blocks with invalid font weights:")
            for item in invalid[:5]:  # Show first 5
                LogUtils.log_block_event(f"     - Slide {item['slide']}, Block: {item['block']}, Weight: {item['invalid_weight']}")
            if len(invalid) > 5:
