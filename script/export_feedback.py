...
class FeedbackRecord:
    ...
    text_amount: str | None
    text_tone: str | None
    audience: str | None
    text_change: str | None
    color: str | None
    font: str | None
    use_web_search: bool | None
    image_style_type: str | None
    slides_count: int | None
    slides_titles: str | None
    user_context: str | None
    file_context: str | None
    user_created_at: datetime | None
    user_presentations_count: int | None
    user_role: str | None
    user_contact: str | None
    presentation_status: str | None
    symbols_used: int | None
    regenerations_left: int | None
    is_archived: bool | None
    generation_error: str | None
    retry_count: int | None
    images_in_process: int | None
    text_regenerations_left: int | None
    image_regenerations_left: int | None
    infographics_generated_left: int | None
    slide_changes_left: int | None
...
class DatabaseService(Protocol):
    ...
    def fetch_feedback_data(self, start_date: date, end_date: date | None = None) -> list[FeedbackRecord]:
...
class DataProcessorImpl:
    ...
    def calculate_statistics(self, records: list[FeedbackRecord]) -> Statistics:
        ...
        unique_presentations = len({record.presentation_id for record in records})
            ...
            with pd.ExcelWriter(filepath, engine="openpyxl") as writer:
                ...
                stats_data = {
                    ...
                    "Количество": [
                        ...
                        stats.dislikes,
                        stats.total,
                        stats.presentations_count,
                    ],
                    "Процент": [
                        f"{stats.likes_percent:.1f}%",
                        f"{stats.neutral_percent:.1f}%",
                        f"{stats.dislikes_percent:.1f}%",
                        "",
                        "",
                    ],
                }
...
class GoogleSheetsExportStrategy:
    ...
    def _prepare_detailed_data(self, feedback_records: list[FeedbackRecord]) -> list[list]:
        ...
        headers = [
            ...
            "Оценка",
            "Комментарий",
            "Использование регенераций",
            "Язык презентации",
            "Стиль изображений и количество слайдов",
        ...
        try:
            ...
            for strategy_name in strategies_to_run:
                if strategy_name in self.export_strategies:
                    ...
                    result = self.export_strategies[strategy_name].export(processed_records, stats, start_date, end_date)
...
class FeedbackExportApplication:
    ...
    def _get_user_confirmation(self, start_date: date, end_date: date | None = None) -> bool:
