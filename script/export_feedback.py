...
class FeedbackExporter:
    ...
    def fetch_feedback_data(self, start_date: date, end_date: Optional[date] = None) -> List[FeedbackRecord]:
        ...
        try:
            with self.connection.cursor(cursor_factory=RealDictCursor) as cursor:
                ...
                date_range_str = f"{start_date.strftime('%d.%m.%Y')} - {end_date.strftime('%d.%m.%Y')}" if start_date != end_date else start_date.strftime('%d.%m.%Y')
    ...
    def calculate_statistics(self, feedback_records: List[FeedbackRecord]) -> Statistics:
        ...
        if not feedback_records:
            return Statistics(
                ...
                avg_symbols=0, avg_text_regenerations=0, avg_image_regenerations=0, avg_infographics=0, avg_slide_changes=0, avg_user_presentations=0,
                download_rate=0.0, error_rate=0.0
            )
        
        total = len(feedback_records)
        likes = sum(1 for record in feedback_records if record.rating == PresentationRating.LIKE)
        neutral = sum(1 for record in feedback_records if record.rating == PresentationRating.NEUTRAL)
        dislikes = sum(1 for record in feedback_records if record.rating == PresentationRating.DISLIKE)
        
        # Count unique presentations
        unique_presentations = len(set(record.presentation_id for record in feedback_records))
        
        # Calculate additional metrics
        symbols_used = [r.symbols_used for r in feedback_records if r.symbols_used is not None]
        avg_symbols = sum(symbols_used) / len(symbols_used) if symbols_used else 0
        
        text_regenerations = [r.text_regenerations_left for r in feedback_records if r.text_regenerations_left is not None]
        avg_text_regenerations = sum(text_regenerations) / len(text_regenerations) if text_regenerations else 0
        
        image_regenerations = [r.image_regenerations_left for r in feedback_records if r.image_regenerations_left is not None]
        avg_image_regenerations = sum(image_regenerations) / len(image_regenerations) if image_regenerations else 0
        
        infographics = [r.infographics_generated_left for r in feedback_records if r.infographics_generated_left is not None]
        avg_infographics = sum(infographics) / len(infographics) if infographics else 0
        
        slide_changes = [r.slide_changes_left for r in feedback_records if r.slide_changes_left is not None]
        avg_slide_changes = sum(slide_changes) / len(slide_changes) if slide_changes else 0
        
        user_presentations = [r.user_presentations_count for r in feedback_records if r.user_presentations_count is not None]
        avg_user_presentations = sum(user_presentations) / len(user_presentations) if user_presentations else 0
        
        downloads = [r.download_count for r in feedback_records if r.download_count is not None]
        download_rate = sum(downloads) / len(downloads) if downloads else 0
        
        errors = [r for r in feedback_records if r.generation_error and r.generation_error != '']
    ...
    def export_to_excel(self, feedback_records: List[FeedbackRecord], stats: Statistics, start_date: date, end_date: Optional[date] = None) -> str:
        ...
        data = [{
            ...
            'Created At': record.created_at.strftime('%d.%m.%Y %H:%M:%S') if record.created_at else 'N/A',
        ...
        logger.info(f"Exported {len(feedback_records)} feedback records to Excel: {filepath}")
        return filepath
    
    def update_sheets(self, feedback_records: List[FeedbackRecord], stats: Statistics, start_date: date, end_date: Optional[date] = None) -> None:
        ...
        try:
            ...
            stats_data = [
                [f"Ежедневная статистика - {sheet_name}"],  # A1 (merged)
            ...
            for record in feedback_records:
                if record.comment:
                    created_info = f" ({record.created_at.strftime('%d.%m.%Y')})" if record.created_at else ""
                    ...
                    settings_info = f" | {user_info} | {performance_info} | {usage_info}"
        ...
        requests = [
            ...
            },
            {
                'updateDimensionProperties': {
                    ...
                    'properties': {
                        'pixelSize': 600  # Column D width for comments
                    },
                    'fields': 'pixelSize'
                }
            },
    ...
    def run_export(self, start_date: date, end_date: Optional[date] = None) -> ExportResult:
        ...
        try:
            ...
            if not feedback_records:
                date_range_str = f"{start_date.strftime('%d.%m.%Y')} - {end_date.strftime('%d.%m.%Y')}" if end_date and start_date != end_date else start_date.strftime('%d.%m.%Y')
                return ExportResult(
                    success=True,
            ...
            excel_file = self.export_to_excel(feedback_records, stats, start_date, end_date)
...
def main():
    ...
    try:
        ...
        if result.success:
            ...
            if result.sheets_updated:
                ...
                if result.statistics:
                    ...
                    print(f"Statistics: {stats.likes} likes ({stats.likes_percent}%), "
        ...
        print(f"\nDate format error: {e}")
        print("Please use format DD.MM.YYYY for single day or DD.MM-DD.MM.YYYY for date range")
        sys.exit(1)
    except Exception as e:
        print(f"\nUnexpected error: {e}")
