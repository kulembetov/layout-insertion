...
async def define_layout_name(message: Message, state: FSMContext):
    ...
    if is_exists:
        await message.answer(
            f"Нашелся шаблон '{layout_name}' в базе данных. Хотите обновить шаблон?",
...
async def insert_choice(query: CallbackQuery, state: FSMContext):
    ...
    match cb_data:
        case "yes":
            logger.info(f"Пользователь {str_user} выбрал опцию 'Да', процесс добавления начат.")
        ...
        case "no":
            ...
            logger.info(f"Пользователь {str_user} выбрал опцию 'Нет', процесс добавления прерван.")
...
async def update_choice(query: CallbackQuery, state: FSMContext):
    ...
    match cb_data:
        case "yes":
            logger.info(f"Пользователь {str_user} выбрал опцию 'Да', процесс обновления начат.")
        ...
        case "no":
            ...
            logger.info(f"Пользователь {str_user} выбрал опцию 'Нет', процесс обновления прерван.")
...
async def insert_logic(query: CallbackQuery, state: FSMContext):
    await query.message.edit_text("Шаблон успешно загружен. Идет процесс добавления...", reply_markup=None)
    ...
    await query.message.answer(f"Шаблон '{layout_name}' был успешно добавлен.")
    await state.clear()
