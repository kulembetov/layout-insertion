...
from django_app.api_v1.utils.text import count_sentences, count_words
...
class BlockBuilder:
    ...
    def __init__(self, block: dict | ExtractedBlock, comments: dict, slide_config: dict | None = None):
        self.block = block
    ...
    def _fill_words(self) -> None:
        text_content: str = self.get("text_content") or ""
    ...
    def __fill_color_var(self) -> None:
        node: dict = self.get("node") or {}
        if node:
            color_hex, color_var = Extractor.extract_color_info(node)
            if color_var:
    ...
    def _fill_by_figure(self) -> None:
        name: str | None = self.get("name")
        clean_name = Extractor.extract_base_figure_name(name)
    ...
    def _extract_figure_info(self) -> dict | None:
    ...
    def _extract_precompiled_image_info(self) -> dict | None:
    ...
    def build(self) -> dict:
        ...
        self._fill_words()
        sql_type: str = self.get("sql_type") or ""
        self._fill_by_sql_type(sql_type)
        self._fill_info()
        return self.block_dict


def block_to_dict(block: ExtractedBlock, comments: dict, slide_config: dict | None = None):
    """
...
def _update_figure_config_with_names(slide_config, blocks):
    ...
    for color_hex, obj_list in slide_config[TYPES.BT_FIGURE].items():
        ...
        for obj in obj_list:
            ...
            figure_obj = {"color": fill_color, "fontFamily": normalized_font_family, "figureName": clean_figure_name}
...
def slide_to_dict(slide: ExtractedSlide, comments: dict) -> dict[str, Any]:
    ...
    for block in slide.blocks:
        if block.sql_type == "text":
            text_content: str = getattr(block, "text_content", "")
